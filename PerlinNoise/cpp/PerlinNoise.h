...
extern "C" {
	__declspec(dllexport) Float* perlinNoise2D(int lattice_x, int lattice_y, int res_x, int res_y);
}
...
// for python usage
extern "C" {
	__declspec(dllexport) Float* perlinNoise3D(int lattice_x, int lattice_y, int lattice_z, int res_x, int res_y, int res_z);
}
